  ...
  public static ScriptTask goBuild() {
    return new ScriptTask()
        ...
        .inlineBody(String.join("\n",
            ...
            "  if [ -f ./main.go ]; then MAIN_GO_PATH=./main.go; fi",
            "fi",
            "if [ -z \"$MAIN_GO_PATH\" ] || [ ! -f \"$MAIN_GO_PATH\" ]; then",
            "  CANDIDATE=$(find . -maxdepth 4 -type f -name main.go | head -1 || true)",
            "  if [ -n \"$CANDIDATE\" ]; then",
            "    MAIN_GO_PATH=\"$CANDIDATE\"",
            "  fi",
            "fi",
            ...
            "  sh -c '",
            "    if [ -z \"${MAIN_GO_PATH}\" ] || [ ! -f \"${MAIN_GO_PATH}\" ]; then",
            "      echo \"Main Go file not found (MAIN_GO_PATH=${MAIN_GO_PATH})\"",
            "      echo \"PWD inside container: $(pwd)\"",
            "      echo \"Tree (maxdepth 4):\"",
            "      find . -maxdepth 4 -type f -name \"*.go\"",
            "      exit 1",
            "    fi && \\",
            "    go env -w GOMODCACHE=/gomod && \\",
            "    go env -w GOCACHE=/gocache && \\",
